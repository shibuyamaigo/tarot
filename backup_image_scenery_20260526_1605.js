...
async function loadImages() {
  try {
    ...
    // キャラクター画像読み込み（normal/fight/tired）
    const characterStates = ['normal', 'fight', 'tired'];
...
class Renderer {
  ...
  drawImagePlayer() {
    ...
    const powerToWeight = power / 70; // 仮定体重70kg
    const cadence = telemetry.cadence;
    
    // 状態判定ロジック
    let characterState = 'normal';
    if (this.state.grade > 3 || powerToWeight > 4.0) {
      characterState = 'fight'; // 登り・高強度時は立ち漕ぎ
    } else if (telemetry.heartRate > 160 || powerToWeight > 6.0) {
      characterState = 'tired'; // 心拍高・超高強度時はしんどい表情
    }
    ...
    switch(characterState) {
      ...
      case 'fight':  
        ...
        break;
      case 'tired':
        animationInterval = 1.4; // 1.4秒間隔（疲労でゆっくり）
        break;
    }
    ...
    const x = this.w * 0.5;
    const y = this.h - 90; // 影により近く、自然な配置に
...
function seedScenery() {
  // より多彩なシーナリー配置
  const types = ['tree', 'tree', 'tree', 'tree', 'tree', 'house', 'sign', 'flower', 'pole'];
